;(() => {
  ...
  function pkgUSDTAmount(pkg) {
    // fixed by marketing plan: 100 / 1000 / 10000
      ...
      if (warnEl) {
        ...
        if (Number(e.lockedUSDT || 0) > 0 && end > 0) {
          warnEl.textContent = (now > end)
            ? "⚠️ Locked หมดอายุแล้ว (เลย 90 วัน) อาจถูกนับเป็น expired ตามกติกาสัญญา"
            : "ℹ️ มี Locked อยู่: จะปลดเมื่อเพดาน (cap) เพิ่มจากการซื้อ/อัปเกรด และต้อง refresh/claim ตามกติกา";
        } else {
          warnEl.textContent = "ℹ️ หากมี Locked: จะปลดได้เมื่อ “เพดาน (cap) เพิ่ม” จากการซื้อ/อัปเกรด และต้อง refresh/claim ตามกติกาสัญญา";
        }
      }
  ...
  async function buyOrUpgrade() {
    ...
    try {
      ...
      if (!ethers.utils.isAddress(sponsor)) {
        toast("Invalid sponsor", "err");
        return;
      }
      if (!ethers.utils.isAddress(placement)) {
        toast("Invalid placement parent", "err");
        return;
      }
      ...
      // After buy, refresh vault + dashboard
      await refreshAll(true);
    } catch (e) {
      console.error(e);
      const msg = e?.data?.message || e?.error?.message || e?.message || String(e);
